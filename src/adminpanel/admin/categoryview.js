...
export default function Categoryview()
{
  ...
  const [items, setItems] = useState([]);
    ...
    async function deleteview(id){
      ...
      try{
        const response=await fetch(`${BACKEND_API}/api/deletecategory`,{
        ...
        <>
         ...
         <div class="container-fluid">
                <div className="card-style mb-30">
                  <h6 className="mb-10"> Category View</h6>
          ...
          {items.map((item , index) => (
            <tr key={item._id}>
              ...
              <td>{item.title}</td>
                ...
                <td><img src={item.pic1} height={100}/></td>
                {/* <td>{item.email}</td>
                <td>{item.phonenumber}</td>
                <td>{item.message}</td> */}
                <td><button className="btn text-danger" onClick={()=>deleteview(item._id)}><i class="lni lni-trash-can"></i></button>
                </td>
                <td><Link to={`/admin/categoryupdate/${item._id}`}><button className="btn border-dark" > UPDATE  </button></Link> </td> 
            </tr>
          ))}
          </tbody>
                    </table>
